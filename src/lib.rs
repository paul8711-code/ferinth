...
//! If you want to use the Modrinth API version 2, which is the latest one currently, specify this
//! crate's major version as `2`.
//!
//! Due to this feature, there will be breaking changes in minor version bumps too!
//!
//! ### Testing
//!
//! Documentation examples are not tested because they require a live Modrinth account. Instead, all
//! functions are verified using up-to-date mock server integration tests.
...
///
/// There are two methods to initialize this container:
///
...
impl Ferinth<()> {
    /// Instantiate the container with the provided [user agent](https://docs.modrinth.com/api/#user-agents) details.
    ///
...
impl Ferinth<Authenticated> {
    /// Instantiate the container with the provided [user agent](https://docs.modrinth.com/api/#user-agents) details and authentication `token`.
    ///
    /// The program `name` is required; `version` and `contact` are optional but recommended.
    ///
